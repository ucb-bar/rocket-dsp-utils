...
trait DspRegisterImp[D, U, EO, EI, B <: Data] extends LazyModuleImp with HasRegMap {
  ...
  val (memMap, memReadEn, memWriteEn) =
    MemToRegmap(streamIn.bits.data.cloneType, mem, true.B, true.B, 24, wordSizeBytes = Some(8))
...
class AXI4DspRegister(val len: Int, val mapMem: Boolean = true, val baseAddr: BigInt = 0, concurrency: Int = 4)(implicit
  p:                           Parameters
) extends AXI4RegisterRouter(baseAddr, beatBytes = 8, concurrency = concurrency)(new AXI4RegBundle(len, _))(
