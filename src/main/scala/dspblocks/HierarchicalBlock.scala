...
abstract class Chain[D, U, EO, EI, B <: Data](blockConstructors: Seq[Parameters => DspBlock[D, U, EO, EI, B]])(implicit
  p:                                                             Parameters
) extends HierarchicalBlock[D, U, EO, EI, B] {
...
class APBChain(blockConstructors: Seq[Parameters => APBDspBlock])(implicit p: Parameters)
    extends Chain(blockConstructors)
    with APBDspBlockWithBus {
  for (b <- blocks) {
