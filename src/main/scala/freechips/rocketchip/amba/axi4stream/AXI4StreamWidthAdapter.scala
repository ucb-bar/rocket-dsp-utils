...
class AXI4StreamWidthAdapter(
  ...
  lazy val module = new LazyModuleImp(this) {
    override def desiredName: String =
      "AXI4StreamWidthAdapater" + (if (nameSuffix.length > 0) {
                                     s"_$nameSuffix"
                                   } else { "" })
...
object AXI4StreamWidthAdapter {
  ...
  def apply(
    ...
    val widthAdapter = LazyModule(
      new AXI4StreamWidthAdapter(
        masterFn = masterFn,
        slaveFn = slaveFn,
  ...
  def nToOneCatAdapter(n: Int): AdapterFun =
    (u, iv, or) => {
      require(n > 0)
      requireIsHardware(u)
  ...
  def nToOneOrAdapater(n: Int): AdapterFun =
    (u, iv, or) => {
  ...
  def oneToNSliceAdapter(n: Int): AdapterFun =
    (d, iv, or) => {
  ...
  def oneToNLastAdapter(n: Int): AdapterFun =
    (d, iv, or) => {
      val cnt = RegInit(0.U(log2Ceil(n).W))
      when(iv && or) { cnt := Mux(cnt === (n - 1).U, 0.U, cnt +& 1.U) }
