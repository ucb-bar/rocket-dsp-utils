...
object AXI4StreamImp
  ...
  def edge(
    pd:         AXI4StreamMasterPortParameters,
    pu:         AXI4StreamSlavePortParameters,
    p:          Parameters,
    sourceInfo: SourceInfo
  ): AXI4StreamEdgeParameters = AXI4StreamEdgeParameters(pd, pu, p, sourceInfo)
  ...
  override def mixO(
    pd:   AXI4StreamMasterPortParameters,
    node: OutwardNode[AXI4StreamMasterPortParameters, AXI4StreamSlavePortParameters, AXI4StreamBundle]
  ): AXI4StreamMasterPortParameters =
    pd.copy(masters = pd.masters.map { c => c.copy(nodePath = node +: c.nodePath) })
  override def mixI(
    pu:   AXI4StreamSlavePortParameters,
    node: InwardNode[AXI4StreamMasterPortParameters, AXI4StreamSlavePortParameters, AXI4StreamBundle]
  ): AXI4StreamSlavePortParameters =
    pu.copy(slaves = pu.slaves.map { m => m.copy(nodePath = node +: m.nodePath) })
}

case class AXI4StreamIdentityNode()(implicit valName: ValName) extends IdentityNode(AXI4StreamImp)()
case class AXI4StreamMasterNode(portParams: Seq[AXI4StreamMasterPortParameters])(implicit valName: ValName)
    extends SourceNode(AXI4StreamImp)(portParams)
...
case class AXI4StreamSlaveNode(portParams: Seq[AXI4StreamSlavePortParameters])(implicit valName: ValName)
    extends SinkNode(AXI4StreamImp)(portParams)
...
case class AXI4StreamNexusNode(
  ...
  slaveFn:          Seq[AXI4StreamSlavePortParameters] => AXI4StreamSlavePortParameters
    ...
    extends NexusNode(AXI4StreamImp)(masterFn, slaveFn)
...
object AXI4StreamAdapterNode {
  def widthAdapter(
    in:                  AXI4StreamMasterPortParameters,
    dataWidthConversion: Int => Int
  ): AXI4StreamMasterPortParameters = {
...
object AXI4StreamAsyncImp
  ...
  override def mixO(
    pd:   AXI4StreamAsyncMasterPortParameters,
    node: OutwardNode[AXI4StreamAsyncMasterPortParameters, AXI4StreamAsyncSlavePortParameters, AXI4StreamAsyncBundle]
  ): AXI4StreamAsyncMasterPortParameters = {
  ...
  override def mixI(
    pu:   AXI4StreamAsyncSlavePortParameters,
    node: InwardNode[AXI4StreamAsyncMasterPortParameters, AXI4StreamAsyncSlavePortParameters, AXI4StreamAsyncBundle]
  ): AXI4StreamAsyncSlavePortParameters = {
...
class BundleBridgeToAXI4Stream(masterParams: AXI4StreamMasterPortParameters)(implicit p: Parameters)
    extends LazyModule {
