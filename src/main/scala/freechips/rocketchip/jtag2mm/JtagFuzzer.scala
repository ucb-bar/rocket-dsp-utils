...
class JtagFuzzer(irLength: Int, beatBytes: Int, numOfTransfers: Int) extends Module {
  ...
  switch(state) {
    ...
    is(State.sTMS) {
      // jtag init
      when(
        (stateCounter === 0.U) || (stateCounter === 2.U) || (stateCounter === 4.U) || (stateCounter === 6.U) || (stateCounter === 8.U)
      ) {
    ...
    is(State.sTCK) {
      // endings
      when(
        (stateCounter === 11.U) || (stateCounter === 31.U) || (stateCounter === 73.U) || (stateCounter === 93.U) || (stateCounter === 167.U)
      ) {
        state := State.sTMS
      }
        // first instruction
